...
class SecurityThreat:
    """Represents a security threat to mature bucks"""
    threat_type: str  # 'road', 'trail', 'parking', 'structure'
    distance: float   # meters from analysis point
    severity: float   # 0.0-1.0 threat level
    confidence: float # 0.0-1.0 confidence in threat assessment

class EnhancedSecurityAnalyzer:
    """Advanced security analysis for pressure-sensitive mature bucks"""
    
    def __init__(self):
        # Security thresholds for mature bucks (more conservative than general deer)
        self.security_thresholds = {
            'major_road_buffer': 1200,      # 1200m from major roads (vs 800m general)
            'minor_road_buffer': 800,       # 800m from minor roads (vs 400m general)
            'structure_buffer': 1000,       # 1000m from buildings (vs 500m general)
            'trail_buffer': 400,            # 400m from established trails (vs 200m general)
            'parking_buffer': 1500,         # 1500m from parking areas (critical for mature bucks)
            'food_plot_buffer': 600,        # 600m from managed food plots (hunting pressure)
        }
        
        # Pressure indicators and their weights
        self.pressure_indicators = {
            'road_density': 0.25,           # Road network density
            'structure_density': 0.20,      # Building/structure density  
            'access_point_density': 0.30,   # Parking, trailheads, gates
            'managed_area_density': 0.15,   # Food plots, stands, feeders
            'trail_network_density': 0.10   # Established trail density
        }
    ...
    def _generate_security_recommendations(self, threat_categories: Dict) -> List[str]:
        """Generate security-based recommendations (DEPRECATED - use _generate_real_security_recommendations)"""
        return ["⚠️ Using legacy recommendation system - update to real OSM data"]
    
    def _get_terrain_security_data(self, lat: float, lon: float, radius_km: float) -> Dict:
        """DEPRECATED - Get terrain data (replaced by real OSM integration)"""
        logger.warning("Using deprecated terrain security data - should use real OSM data")
        return {}
    
    def _calculate_analysis_confidence(self, terrain_data: Dict) -> float:
        """DEPRECATED - Calculate confidence (replaced by real data confidence)"""
        return 0.2  # Low confidence for deprecated method
