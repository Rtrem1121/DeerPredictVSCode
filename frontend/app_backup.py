...
if 'prediction' in st.session_state and st.session_state.prediction:
    ...
    with st.expander("📖 How to Read These Heatmaps", expanded=False):
        st.markdown("""
        **🎨 NEW ENHANCED COLOR SCHEMES:**
        
        **🚶 Travel Corridors**: Dark Purple = Low movement → Bright Pink/Yellow = High movement
        **🛏️ Bedding Areas**: Dark Blue = Low bedding → Bright Yellow/Green = High bedding  
        **🌾 Feeding Areas**: Black = Low feeding → Bright White/Yellow = High feeding
        
        **📊 Reading the Colors:**
        - **Dark/Black areas**: Very low deer activity (avoid these)
        - **Medium brightness**: Moderate activity (backup options)  
        - **Bright/Light areas**: High activity (PRIME HUNTING SPOTS!)
        
        **🎯 Mature Buck Strategy:**
        - Target bright areas in bedding maps (security zones)
        - Hunt transition zones between bright bedding and feeding areas
        - Focus on bright travel corridors connecting bedding to feeding
        """)
