...
try:
    camera_placement_system = BuckCameraPlacement("http://localhost:8000")
    CAMERA_PLACEMENT_AVAILABLE = True
    logger.info("🎥 Camera placement system initialized successfully")
except Exception as e:
    CAMERA_PLACEMENT_AVAILABLE = False
    logger.warning(f"Camera placement system not available: {e}")
...
def predict_movement(request: PredictionRequest):
    ...
    try:
        ...
        # 11. Calculate optimal camera placement if requested and available
        optimal_camera_placement = None
        if request.include_camera_placement and CAMERA_PLACEMENT_AVAILABLE:
            try:
                logger.info("🎥 Calculating optimal camera placement for prediction...")
                camera_result = camera_placement_system.calculate_optimal_camera_position(request.lat, request.lon)
                
                # Format camera placement for integration with deer prediction
                camera = camera_result["optimal_camera"]
                strategy = camera_result["placement_strategy"]
                
                optimal_camera_placement = {
                    "enabled": True,
                    "camera_coordinates": {
                        "lat": camera["lat"],
                        "lon": camera["lon"]
                    ...
                    "distance_meters": camera["distance_from_target_meters"],
                    "placement_reasoning": strategy["primary_factors"][0] if strategy["primary_factors"] else "Optimal positioning",
                    "optimal_times": strategy["optimal_times"],
                    "detection_range": strategy["expected_detection_range"],
                    "integration_notes": [
                        f"Camera positioned {camera['distance_from_target_meters']:.0f}m from your target location",
                        f"Strategic placement based on {strategy['placement_type']} principles",
                        f"Expected to capture {strategy['optimal_times'][0]} and {strategy['optimal_times'][1]} movement",
                        "Coordinates ready for GPS navigation to setup location"
                    ],
                    "data_quality": camera_result["satellite_data_quality"],
                    "timestamp": camera_result["analysis_timestamp"]
                }
                
                # Add camera placement info to notes
                notes += f"\n\n🎥 **Optimal Camera Placement** (Advanced Analysis):\n"
                notes += f"📍 **Camera GPS**: ({camera['lat']:.6f}, {camera['lon']:.6f})\n"
                notes += f"📏 **Distance**: {camera['distance_from_target_meters']:.0f} meters from target\n"
                notes += f"🎯 **Confidence**: {camera['confidence_score']:.1f}%\n"
                notes += f"💭 **Strategy**: {strategy['primary_factors'][0] if strategy['primary_factors'] else 'Optimal positioning'}\n"
                notes += f"⏰ **Best Times**: {', '.join(strategy['optimal_times'])}\n"
                notes += f"📡 **Detection Range**: {strategy['expected_detection_range']}\n"
                
                logger.info(f"✅ Camera placement calculated: {camera['confidence_score']:.1f}% confidence")
                
            except Exception as e:
                logger.warning(f"Camera placement calculation failed: {e}")
                optimal_camera_placement = {
                    "enabled": False,
                    "error": f"Camera placement calculation failed: {str(e)}"
                }
        elif request.include_camera_placement and not CAMERA_PLACEMENT_AVAILABLE:
            optimal_camera_placement = {
                "enabled": False,
                "error": "Camera placement system not available"
            }
        ...
        response = PredictionResponse(
            ...
            optimal_camera_placement=optimal_camera_placement  # Add camera placement if requested
        )
...
@app.post("/api/camera/optimal-placement", summary="Get optimal single camera placement", tags=["hunting"])
def get_optimal_camera_placement(request: CameraPlacementRequest):
    """
    Get the single optimal trail camera placement using advanced satellite analysis
    
    This endpoint uses the same advanced algorithms as your deer predictions
    to calculate the mathematically optimal camera position for mature buck photography.
    """
    if not CAMERA_PLACEMENT_AVAILABLE:
        raise HTTPException(status_code=503, detail="Camera placement system not available")
    
    try:
        logger.info(f"🎥 Advanced camera placement request: lat={request.lat}, lon={request.lon}")
        
        # Use the advanced camera placement system
        result = camera_placement_system.calculate_optimal_camera_position(request.lat, request.lon)
        ...
        camera_feature = {
            ...
            "properties": {
                ...
                "distance_from_target": f"{camera['distance_from_target_meters']:.0f} meters",
                "reasoning": strategy["primary_factors"][0] if strategy["primary_factors"] else "Optimal positioning",
                "placement_strategy": strategy["placement_type"],
                "optimal_times": strategy["optimal_times"],
                "detection_range": strategy["expected_detection_range"],
                "setup_notes": [
                    f"Position {camera['distance_from_target_meters']:.0f}m from target location",
                    "Use scent-free setup procedures",
                    "Clear 20-30 yard shooting lanes",
                    "Mount 10-12 feet high, angled 30° downward"
                ],
                "data_source": quality.get("data_source", "satellite_enhanced")
            }
        }
        ...
        target_feature = {
            ...
            "properties": {
                ...
                "description": "Your selected hunting target"
            }
        }
        
        camera_geojson = {
            "type": "FeatureCollection",
            "features": [target_feature, camera_feature]
        }
        
        # Generate insights
        insights = []
        if camera["confidence_score"] >= 85:
            insights.append(f"🔥 Excellent placement: {camera['confidence_score']:.1f}% confidence")
        elif camera["confidence_score"] >= 75:
            insights.append(f"⭐ Very good placement: {camera['confidence_score']:.1f}% confidence")
        else:
            insights.append(f"✅ Good placement: {camera['confidence_score']:.1f}% confidence")
            
        insights.append(f"📏 Optimal distance: {camera['distance_from_target_meters']:.0f} meters from target")
        insights.append(f"🧠 Strategy: {strategy['primary_factors'][0] if strategy['primary_factors'] else 'Mathematically optimized'}")
        
        if quality.get("data_source") == "google_earth_engine":
            insights.append("🛰️ Enhanced with real-time satellite vegetation data")
        
        response = {
            "camera_placement": camera_geojson,
            "target_location": {"lat": request.lat, "lon": request.lon},
            "optimal_camera": {
                "lat": camera["lat"],
                "lon": camera["lon"], 
                "confidence_score": camera["confidence_score"],
            ...
            "placement_analysis": {
                "strategy": strategy["placement_type"],
                "primary_factors": strategy["primary_factors"],
                "optimal_times": strategy["optimal_times"],
                "detection_range": strategy["expected_detection_range"]
            },
            "data_quality": {
                "source": quality.get("data_source", "standard"),
                "timestamp": result["analysis_timestamp"]
            },
            "insights": insights,
            "usage_instructions": {
                "setup_height": "10-12 feet",
                "setup_angle": "30° downward toward target",
                "trigger_speed": "0.3 seconds or faster",
                "detection_range": "80+ feet",
                "camera_settings": "Burst mode, low-glow IR",
                "best_times": strategy["optimal_times"]
            }
        }
        
        logger.info(f"✅ Advanced camera placement completed: {camera['confidence_score']:.1f}% confidence")
        return response
        
    except Exception as e:
        logger.error(f"Advanced camera placement failed: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Camera placement calculation failed: {str(e)}")
