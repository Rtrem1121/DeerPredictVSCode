...
class PredictionService:
    """
    Enhanced prediction service using EnhancedBeddingZonePredictor as primary engine.
    
    This service now prioritizes the enhanced biological analysis over traditional algorithms
    to achieve the target 75.3% suitability with multiple bedding zones.
    """
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        
        # PRIMARY: Initialize EnhancedBeddingZonePredictor as main prediction engine
        try:
            self.predictor = EnhancedBeddingZonePredictor()
            self.logger.info("✅ EnhancedBeddingZonePredictor initialized as primary predictor")
        except Exception as e:
            self.logger.error(f"❌ Failed to initialize EnhancedBeddingZonePredictor: {e}")
            raise
        
        # Secondary systems for compatibility
        ...
        # Initialize scouting enhancement
        try:
            from backend.scouting_prediction_enhancer import get_scouting_enhancer
            self.scouting_enhancer = get_scouting_enhancer()
        except ImportError:
            self.logger.warning("Scouting enhancer not available")
            self.scouting_enhancer = None
        
        self.logger.info("✅ PredictionService initialized with EnhancedBeddingZonePredictor as primary engine")
    ...
    def predict(self, context: PredictionContext) -> PredictionResult:
        ...
        try:
            ...
            logger.info("🔍 DEBUG: About to call _execute_rule_engine")
            score_maps = self._execute_rule_engine(context, terrain_data, weather_data)
            logger.info("🔍 DEBUG: _execute_rule_engine completed")
    ...
    def _execute_rule_engine(self, context: PredictionContext, terrain_data: Dict[str, Any], 
        ...
        """Execute the deer behavior rule engine with enhanced bedding zone integration"""
        try:
            ...
            # DEBUG: Check enhanced bedding predictor status
            logger.info(f"🔍 DEBUG: Enhanced bedding predictor exists: {self.enhanced_bedding_predictor is not None}")
            
            # ENHANCED: Replace bedding zones with biologically accurate prediction
            if self.enhanced_bedding_predictor:
                try:
                    logger.info("🛏️ Generating enhanced bedding zones...")
                    
                    # Get enhanced environmental data for bedding zone analysis
                    gee_data = self.enhanced_bedding_predictor.get_dynamic_gee_data_enhanced(
                        context.lat, context.lon
                    )
                    osm_data = self.enhanced_bedding_predictor.get_osm_road_proximity(
                        context.lat, context.lon
                    )
                    enhanced_weather = self.enhanced_bedding_predictor.get_enhanced_weather_with_trends(
                        context.lat, context.lon
                    )
                    
                    # Generate enhanced bedding zones
                    enhanced_bedding_zones = self.enhanced_bedding_predictor.generate_enhanced_bedding_zones(
                        context.lat, context.lon, gee_data, osm_data, enhanced_weather
                    )
                    
                    # Convert bedding zones to score map if zones were generated
                    bedding_features = enhanced_bedding_zones.get("features", [])
                    if bedding_features:
                        # Create enhanced bedding score map from feature locations
                        ...
                        # Replace original bedding scores with enhanced ones
                        score_maps['bedding'] = enhanced_bedding_scores
                        
                        # Cache enhanced bedding zones for later use
                        self._cached_enhanced_bedding_zones = enhanced_bedding_zones
                        
                        logger.info(f"✅ Enhanced bedding zones integrated: {len(bedding_features)} zones generated")
                    else:
                        logger.warning("❌ No enhanced bedding zones generated, using original scores")
                        self._cached_enhanced_bedding_zones = None
                    
                except Exception as e:
                    logger.warning(f"🛏️ Enhanced bedding zone generation failed: {e}, using original scores")
                    self._cached_enhanced_bedding_zones = None
    ...
    def _convert_bedding_zones_to_scores(self, bedding_features: List[Dict], 
                                       center_lat: float, center_lon: float, 
                                       target_shape: Tuple[int, int]) -> np.ndarray:
        """Convert enhanced bedding zone features to score map grid"""
        try:
            # Initialize score map
            score_map = np.zeros(target_shape)
            
            if not bedding_features:
                return score_map
            
            # Calculate grid parameters
            grid_height, grid_width = target_shape
            lat_range = 0.02  # Approximate degree range for grid
            lon_range = 0.02
            
            for feature in bedding_features:
                try:
                    # Get feature coordinates and score
                    coords = feature["geometry"]["coordinates"]
                    properties = feature["properties"]
                    
                    zone_lon, zone_lat = coords[0], coords[1]
                    zone_score = properties.get("score", 0.5)
                    suitability_score = properties.get("suitability_score", 50) / 100.0
                    
                    # Convert lat/lon to grid indices
                    lat_offset = zone_lat - center_lat
                    lon_offset = zone_lon - center_lon
                    
                    # Calculate grid position (center grid at middle of array)
                    grid_i = int((lat_offset / lat_range) * grid_height + grid_height // 2)
                    grid_j = int((lon_offset / lon_range) * grid_width + grid_width // 2)
                    
                    # Ensure indices are within bounds
                    if 0 <= grid_i < grid_height and 0 <= grid_j < grid_width:
                        # Use the higher of zone_score and suitability_score
                        final_score = max(zone_score, suitability_score)
                        score_map[grid_i, grid_j] = final_score
                        
                        # Add influence to surrounding cells (bedding areas have some spread)
                        influence_radius = 1
                        for di in range(-influence_radius, influence_radius + 1):
                            for dj in range(-influence_radius, influence_radius + 1):
                                ...
                                if 0 <= ni < grid_height and 0 <= nj < grid_width:
                                    ...
                                    if distance > 0:
                                        influence = final_score * (0.7 / distance)
                                        score_map[ni, nj] = max(score_map[ni, nj], influence)
                
                except Exception as e:
                    logger.warning(f"Failed to convert bedding feature to grid: {e}")
                    continue
            
            logger.debug(f"Converted {len(bedding_features)} bedding zones to score map: max={np.max(score_map):.3f}")
            return score_map
            
        except Exception as e:
            logger.error(f"Failed to convert bedding zones to scores: {e}")
    ...
    def _build_prediction_result(self, context: PredictionContext, terrain_data: Dict[str, Any],
        ...
        """Build the final prediction result object with enhanced bedding zones"""
        try:
            ...
            # ENHANCED: Use cached enhanced bedding zones if available, otherwise use score map
            if hasattr(self, '_cached_enhanced_bedding_zones') and self._cached_enhanced_bedding_zones:
                bedding_zones = self._cached_enhanced_bedding_zones
                logger.info("✅ Using enhanced bedding zones in final result")
            else:
                bedding_zones = self.core.generate_zones_from_scores(score_maps['bedding'])
                logger.info("ℹ️ Using traditional bedding zones in final result")
    ...
    def _generate_heatmaps(self, terrain_data: Dict[str, Any], 
        ...
        try:
            # Convert numpy arrays to JSON strings for frontend
            heatmaps = {
                'terrain': json.dumps(score_maps.get('travel', np.zeros((6, 6))).tolist()),
                'vegetation': json.dumps(score_maps.get('bedding', np.zeros((6, 6))).tolist()),
                'travel_score': json.dumps(score_maps.get('travel', np.zeros((6, 6))).tolist()),
                'bedding_score': json.dumps(score_maps.get('bedding', np.zeros((6, 6))).tolist()),
                'feeding_score': json.dumps(score_maps.get('feeding', np.zeros((6, 6))).tolist())
            }
            ...
            return {
                'terrain': json.dumps(np.zeros((6, 6)).tolist()),
                'vegetation': json.dumps(np.zeros((6, 6)).tolist()),
                'travel_score': json.dumps(np.zeros((6, 6)).tolist()),
                'bedding_score': json.dumps(np.zeros((6, 6)).tolist()),
                'feeding_score': json.dumps(np.zeros((6, 6)).tolist())
            }
    
    def _get_default_weather_data(self) -> Dict[str, Any]:
        """Get default weather data when API fails"""
        return {
            'temperature': 45,  # Default Vermont temperature
    ...
    def _get_default_mature_buck_data(self) -> Dict[str, Any]:
        """Get default mature buck data when analysis fails"""
        return {
            'viable_location': False,
            'confidence_summary': {
                'overall_suitability': 40.0,
                'movement_confidence': 35,
                'pressure_tolerance': 30.0
            },
            'opportunity_markers': [],
            'stand_recommendations': [],
            'terrain_scores': {
                'bedding_suitability': 40.0,
                'escape_route_quality': 35.0,
                'isolation_score': 45.0,
                'pressure_resistance': 30.0,
                'overall_suitability': 37.5
            },
            'movement_prediction': {
                'movement_probability': 35.0,
                'confidence_score': 35.0,
                'preferred_times': ['Dawn', 'Dusk'],
                'behavioral_notes': ['Limited activity expected', 'Basic movement patterns']
            },
            'camera_placement': {
                'lat': 0,
                'lon': 0,
    ...
    def predict_movement(self, request: PredictionRequest) -> PredictionResponse:
        ...
        try:
            ...
            # 3.25. ENHANCED BEDDING ZONE INTEGRATION - PRIORITY SYSTEM
            logger.info("🔍 ENHANCED BEDDING INTEGRATION: Checking predictor availability...")
            enhanced_bedding_success = False
            
            if self.enhanced_bedding_predictor:
                logger.info("✅ ENHANCED BEDDING PREDICTOR: Available, starting integration...")
                try:
                    logger.info("🛏️ ENHANCED INTEGRATION: Running enhanced biological analysis...")
                    
                    # Use the enhanced biological analysis directly for better integration
                    enhanced_result = self.enhanced_bedding_predictor.run_enhanced_biological_analysis(
                        request.lat, request.lon, current_hour, request.season, 'low'
                    )
                    
                    # Extract enhanced bedding zones
                    enhanced_bedding_zones = enhanced_result.get("bedding_zones", {})
                    bedding_features = enhanced_bedding_zones.get("features", [])
                    
                    if bedding_features:
                        # PRIORITY: Replace original bedding scores with enhanced analysis
                        enhanced_bedding_scores = self._convert_bedding_zones_to_scores(
                            bedding_features, request.lat, request.lon, score_maps['bedding'].shape
                        )
                        
                        # FULL REPLACEMENT: Use enhanced bedding scores instead of original
                        score_maps['bedding'] = enhanced_bedding_scores
                        enhanced_bedding_success = True
                        
                        # Cache enhanced data for async predict method
                        self._cached_enhanced_bedding_zones = enhanced_bedding_zones
                        
                        # Extract comprehensive metrics for logging
                        suitability_analysis = enhanced_bedding_zones.get('properties', {}).get('suitability_analysis', {})
                        overall_suitability = suitability_analysis.get('overall_score', 0)
                        confidence_score = enhanced_result.get('confidence_score', 0)
                        
                        # ENHANCED SUCCESS LOGGING
                        logger.info(f"🎯 ENHANCED BEDDING SUCCESS: {len(bedding_features)} zones generated")
                        logger.info(f"📊 ENHANCED METRICS: Suitability={overall_suitability:.1f}%, Confidence={confidence_score:.1f}%")
                        logger.info(f"🔄 BEDDING REPLACEMENT: Enhanced scores max={np.max(enhanced_bedding_scores):.2f}")
                        
                        # Log detailed zone information
                        for i, feature in enumerate(bedding_features[:3]):  # Show first 3 zones
                            props = feature.get('properties', {})
                            coords = feature.get('geometry', {}).get('coordinates', [0, 0])
                            zone_score = props.get('suitability_score', props.get('score', 0))
                            logger.info(f"   🛌 Zone {i+1}: {zone_score:.1f}% at {coords[1]:.4f}, {coords[0]:.4f}")
                        
                    else:
                        logger.warning("⚠️ ENHANCED BEDDING: No zones generated, keeping original scores")
                        
                except Exception as e:
                    logger.error(f"❌ ENHANCED BEDDING FAILED: {e}")
                    logger.error(f"   Falling back to original bedding algorithm")
                    enhanced_bedding_success = False
            else:
                logger.warning("⚠️ ENHANCED BEDDING PREDICTOR: Not available, using original algorithm")
            
            # Log integration status
            if enhanced_bedding_success:
                logger.info("✅ BEDDING INTEGRATION: Enhanced predictor successfully integrated")
            else:
                logger.info("🔄 BEDDING INTEGRATION: Using original algorithm as fallback")

            # 3.5. ENHANCE PREDICTIONS WITH SCOUTING DATA
            logger.info("Enhancing predictions with scouting data...")
            enhancement_result = {"enhancements_applied": [], "mature_buck_indicators": 0}
            try:
                if self.scouting_enhancer:
                    enhancement_result = self.scouting_enhancer.enhance_predictions(
    ...
    def _markers_to_geojson(self, markers: List[Dict[str, Any]], marker_type: str) -> Dict[str, Any]:
        """
        Convert markers to GeoJSON format for frontend mapping.
        
        Args:
            markers: List of marker dictionaries with lat, lon, score, etc.
            marker_type: Type of markers ('travel', 'bedding', 'feeding', etc.)
            
        Returns:
            Dict in GeoJSON format with features array
        """
        try:
            features = []
            
            for i, marker in enumerate(markers):
                if not marker.get('lat') or not marker.get('lon'):
                    continue
                ...
                feature = {
                    ...
                    "geometry": {
                        "type": "Point",
                        "coordinates": [marker['lon'], marker['lat']]  # GeoJSON is [lon, lat]
                    },
                    "properties": {
                        "id": f"{marker_type}_{i}",
                        "type": marker_type,
                        "score": marker.get('score', 0),
                        "confidence": marker.get('confidence', 0),
                        "description": marker.get('description', f'{marker_type.title()} zone'),
                        "marker_index": i
                    }
                }
                
                # Add type-specific properties
                if marker_type == 'travel':
                    feature['properties']['corridor_type'] = marker.get('corridor_type', 'general')
                    feature['properties']['usage_frequency'] = marker.get('usage_frequency', 'moderate')
                elif marker_type == 'bedding':
                    feature['properties']['security_level'] = marker.get('security_level', 'medium')
                    feature['properties']['capacity'] = marker.get('capacity', 'small')
                elif marker_type == 'feeding':
                    feature['properties']['food_type'] = marker.get('food_type', 'natural')
                    feature['properties']['season_availability'] = marker.get('season_availability', 'year_round')
            ...
            logger.error(f"Failed to convert {marker_type} markers to GeoJSON: {e}")
    ...
    async def predict(self, lat: float, lon: float, time_of_day: int, season: str, hunting_pressure: str) -> Dict:
        """
        PRIMARY PREDICTION METHOD using EnhancedBeddingZonePredictor
        
        This method uses the enhanced biological analysis as the main prediction engine
        to achieve the target 75.3% suitability with multiple bedding zones.
        """
        try:
            # Use EnhancedBeddingZonePredictor as primary prediction engine
            result = self.predictor.run_enhanced_biological_analysis(
                lat, lon, time_of_day, season, hunting_pressure
            )
            
            # Extract enhanced bedding zones (primary output)
            bedding_zones = result["bedding_zones"]
            num_zones = len(bedding_zones['features']) if bedding_zones and 'features' in bedding_zones else 0
            
            # Get overall suitability from enhanced analysis
            suitability_analysis = bedding_zones.get('properties', {}).get('suitability_analysis', {}) if bedding_zones else {}
            overall_suitability = suitability_analysis.get('overall_score', 0)
            confidence = result.get('confidence_score', 0)
            
            # Log enhanced prediction results
            self.logger.info(f"🎯 Enhanced Prediction Results:")
            self.logger.info(f"   📍 Location: {lat:.4f}, {lon:.4f}")
            self.logger.info(f"   🛌 Bedding Zones: {num_zones}")
            self.logger.info(f"   📊 Overall Suitability: {overall_suitability:.1f}%")
            self.logger.info(f"   🎯 Confidence: {confidence:.2f}")
            
            # Log individual zone details
            if bedding_zones and 'features' in bedding_zones:
                for i, zone in enumerate(bedding_zones['features'][:3], 1):  # Show top 3
                    zone_props = zone.get('properties', {})
                    zone_coords = zone.get('geometry', {}).get('coordinates', [0, 0])
                    zone_suitability = zone_props.get('suitability_score', 0)
                    self.logger.info(f"   🛌 Zone {i}: {zone_suitability:.1f}% at {zone_coords[1]:.4f}, {zone_coords[0]:.4f}")
