...
try:
    ...
    from lidar_processor import get_lidar_processor, DEMFileManager, TerrainExtractor, BatchLIDARProcessor
...
class EnhancedBeddingZonePredictor(OptimizedBiologicalIntegration):
    ...
    def get_dynamic_gee_data_enhanced(self, lat: float, lon: float, vegetation_data: Optional[Dict] = None, max_retries: int = 5, prefer_lidar: bool = True) -> Dict:
        ...
        if vegetation_data and 'canopy_coverage_analysis' in vegetation_data:
            ...
            if 'ndvi_trend' in vegetation_data:
    ...
    def evaluate_bedding_suitability(self, gee_data: Dict, osm_data: Dict, weather_data: Dict) -> Dict:
        ...
        # Movement corridor score (benches/saddles from LiDAR when available)
        lidar_terrain = gee_data.get("lidar_terrain", {}) if isinstance(gee_data.get("lidar_terrain"), dict) else {}
        benches_count = int(lidar_terrain.get("benches_count", 0))
        saddles_count = int(lidar_terrain.get("saddles_count", 0))
        if benches_count or saddles_count:
            corridor_score = min(100, 40 + benches_count * 10 + saddles_count * 20)
        else:
            # Fallback heuristic: moderate slopes tend to form travel benches/corridors
            corridor_score = 80 if 6 <= criteria["slope"] <= 18 else 50
    ...
    def _apply_scouting_priors_to_bedding_zones(self, bedding_zones: Dict, center_lat: float, center_lon: float) -> Dict[str, Any]:
        """Boost bedding zones near recent scouting observations."""
        summary = {
            "observations_used": 0,
            "total_boost": 0.0,
            "max_boost": 0.0
        }
        if not bedding_zones or not isinstance(bedding_zones.get("features"), list):
            return summary

        try:
            from backend.scouting_models import ScoutingQuery, ObservationType
            from backend.scouting_data_manager import get_scouting_data_manager
            from datetime import datetime, timezone
        except Exception as e:
            logger.debug(f"Scouting priors unavailable: {e}")
            return summary

        boost_by_type = {
            ObservationType.BEDDING_AREA: 12.0,
            ObservationType.RUB_LINE: 10.0,
            ObservationType.FRESH_SCRAPE: 10.0,
            ObservationType.TRAIL_CAMERA: 8.0,
            ObservationType.DEER_TRACKS: 6.0,
            ObservationType.FEEDING_SIGN: 5.0
        }

        query = ScoutingQuery(lat=center_lat, lon=center_lon, radius_miles=5.0, days_back=365)
        observations = get_scouting_data_manager().get_observations(query)
        if not observations:
            return summary
        ...
        for feature in bedding_zones["features"]:
            ...
            for obs in observations:
                base_boost = boost_by_type.get(obs.observation_type, 0.0)
                if base_boost <= 0:
                    continue
                ...
                distance_factor = max(0.2, 1.0 - distance_m / 250.0)

                boost = base_boost * decay * distance_factor
                if boost <= 0:
                    continue
            ...
            if zone_boost > 0:
                ...
                props["scouting_boost"] = round(zone_boost, 2)
                props["scouting_observations_used"] = obs_used
                props["score"] = min(100, props.get("score", 0) + zone_boost)
                props["suitability_score"] = min(100, props.get("suitability_score", props.get("score", 0)) + zone_boost)
                summary["observations_used"] += obs_used
                summary["total_boost"] += zone_boost
                summary["max_boost"] = max(summary["max_boost"], zone_boost)
    ...
    def run_enhanced_biological_analysis(self, lat: float, lon: float, time_of_day: int,
        ...
        scouting_prior_summary = self._apply_scouting_priors_to_bedding_zones(bedding_zones, lat, lon)
        if isinstance(bedding_zones.get("properties"), dict):
            bedding_zones["properties"]["scouting_prior_summary"] = scouting_prior_summary
        
        # REMOVED: Old stand generation system - now using mature_buck_points_generator.py exclusively
        # This eliminates duplicate stand systems and uses only the new interception stand strategy
        ...
        # )
        stand_recommendations = []  # Empty - interception stands now come from mature_buck_points_generator.py
        ...
        result = {
            ...
            "bedding_zones": bedding_zones,
            "trace_summary": self.last_scan_trace or {},
            "scouting_prior_summary": scouting_prior_summary,
            "mature_buck_analysis": {
                "stand_recommendations": stand_recommendations,
                "terrain_scores": {
                    "bedding_suitability": gee_data.get('canopy_coverage', 0.5) * 100,
                    "escape_route_quality": min(100, osm_data.get('nearest_road_distance_m', 500) / 5),
                    "isolation_score": min(100, osm_data.get('nearest_road_distance_m', 500) / 5),
                    "pressure_resistance": max(0, 100 - (osm_data.get('road_density', 0) * 50)),
                    "overall_suitability": confidence
                },
                "movement_prediction": {
    ...
    def generate_enhanced_stand_sites(self, lat: float, lon: float, gee_data: Dict, 
        ...
        try:
            ...
            try:
                ...
                enhanced_placement = get_enhanced_stand_placement()
                
                # Prepare comprehensive data for enhanced analysis
                terrain_analysis = {
                    'elevation': gee_data.get('elevation', 382),
                    'slope': gee_data.get('slope', 15.0),
                    'aspect': gee_data.get('aspect', 25.0),
                    'canopy_coverage': gee_data.get('canopy_coverage', 0.5),
                    'ndvi_value': gee_data.get('ndvi_value', 0.5),
                    'confidence': 0.8
                }
                ...
                security_analysis = {
                    'road_distance': osm_data.get('nearest_road_distance_m', 500),
                    'isolation_score': min(100, osm_data.get('nearest_road_distance_m', 500) / 5),
                    'human_pressure': max(0, 100 - (osm_data.get('road_density', 0) * 50)),
                    'confidence': 0.7
                }
                
                food_analysis = {
                    'vegetation_quality': gee_data.get('deciduous_forest_percentage', 0.5) * 100,
                    'ndvi_score': gee_data.get('ndvi_value', 0.5) * 100,
                    'seasonal_availability': 75 if season in ['fall', 'rut'] else 60,
                    'confidence': 0.6
                }
                ...
                stand_sites = []
                candidate_offsets = [
                    (0.0008, 0.0008),   # NE - Primary position
                    (-0.0008, 0.0008),  # SE - Secondary position  
                    (0.0008, -0.0008),  # NW - Security position
                ]
                
                for i, (lat_offset, lon_offset) in enumerate(candidate_offsets):
                    site_lat = lat + lat_offset
                    site_lon = lon + lon_offset
                    
                    # Analyze this specific stand site
                    site_analysis = enhanced_placement._analyze_stand_site(
                        site_lat, site_lon, terrain_analysis, security_analysis, food_analysis, season
                    )
                    
                    # Determine stand type based on primary advantage
                    stand_types = {
                        'security': 'Security Stand',
                        'food': 'Food Source Stand', 
                        'bedding': 'Bedding Proximity Stand',
                        'travel': 'Travel Corridor Stand',
                        'wind': 'Wind Advantage Stand'
                    }
                    stand_type = stand_types.get(site_analysis['primary_advantage'], 'Strategic Stand')
                    
                    # Calculate distance from bedding zones
                    distance_from_bedding = 200  # Default
                    if bedding_zones and bedding_zones.get('features'):
                        bedding_feature = bedding_zones['features'][0]
                        bedding_coords = bedding_feature.get('geometry', {}).get('coordinates', [lon, lat])
                    ...
                    stand_site = {
                        ...
                        "properties": {
                            "hunt_type": stand_type,
                            "confidence": f"{site_analysis['overall_score']:.1f}%",
                            "distance_from_bedding": f"{distance_from_bedding:.0f}m",
                            "primary_advantage": site_analysis['primary_advantage'],
                            "security_score": f"{site_analysis['security_score']:.1f}%",
                            "food_score": f"{site_analysis['food_score']:.1f}%",
                            "bedding_score": f"{site_analysis['bedding_score']:.1f}%",
                            "travel_score": f"{site_analysis['travel_score']:.1f}%",
                            "wind_favorability": f"{site_analysis['wind_favorability']:.1f}%",
                            "recommendations": site_analysis.get('recommendations', []),
                            "risk_factors": site_analysis.get('risk_factors', []),
                            "methodology": "EnhancedStandPlacement v1.0",
                            "analysis_confidence": f"{site_analysis['confidence']:.1f}%"
                        }
                    }
    ...
    def _generate_dense_lidar_candidates(self,
                                         center_lat: float,
                                         center_lon: float,
                                         radius_m: int,
                                         target_points: int) -> List[Dict[str, Any]]:
        """
        Generate a dense, deterministic grid of candidate points within a radius.

        Uses a spacing derived from target_points to approximate the requested density.
        Returns candidates with offsets to preserve distance calculations downstream.
        """
        if target_points <= 0 or radius_m <= 0:
            return []

        area_m2 = math.pi * (radius_m ** 2)
        spacing_m = max(3.0, math.sqrt(area_m2 / max(target_points, 1)))

        lat_to_meters = 111000
        lon_to_meters = 111000 * math.cos(math.radians(center_lat))

        max_steps = int(radius_m / spacing_m)
        candidates: List[Dict[str, Any]] = []

        for y_step in range(-max_steps, max_steps + 1):
            dy_m = y_step * spacing_m
            for x_step in range(-max_steps, max_steps + 1):
                dx_m = x_step * spacing_m
                if (dx_m * dx_m + dy_m * dy_m) > (radius_m * radius_m):
                    continue

                lat_offset = dy_m / lat_to_meters
                lon_offset = dx_m / lon_to_meters
                candidates.append({
                    "lat": center_lat + lat_offset,
                    "lon": center_lon + lon_offset,
                    "tier": {"name": "dense_lidar"},
                    "lat_offset": lat_offset,
                    "lon_offset": lon_offset,
                    "distance_m": int((dx_m * dx_m + dy_m * dy_m) ** 0.5)
                })

        if len(candidates) > target_points:
            stride = max(1, math.ceil(len(candidates) / target_points))
            candidates = candidates[::stride]

        return candidates

    def _decluster_candidates(self,
                              candidates: List[Dict[str, Any]],
                              min_separation_m: int,
                              max_keep: int) -> List[Dict[str, Any]]:
        """Reduce overlapping candidates by keeping top-scored points far enough apart."""
        if not candidates:
            return []
        ...
        for candidate in candidates:
            ...
            keep = True
            cand_lat = candidate["candidate"]["lat"]
            cand_lon = candidate["candidate"]["lon"]
            for chosen in selected:
                chosen_lat = chosen["candidate"]["lat"]
                chosen_lon = chosen["candidate"]["lon"]
                distance_m = ((cand_lat - chosen_lat) ** 2 + (cand_lon - chosen_lon) ** 2) ** 0.5 * 111000
    ...
    def _search_alternative_bedding_sites(self, center_lat: float, center_lon: float, 
        ...
        try:
            ...
            lidar_config = {}
            if self.config:
                lidar_config = self.config.get("lidar_dense_scan", {}) or {}

            dense_scan_enabled = str(lidar_config.get("enabled", "false")).lower() == "true"
            dense_scan_points = int(lidar_config.get("points", 200000))
            dense_scan_radius_m = int(lidar_config.get("radius_m", 800))
            dense_scan_top_k = int(lidar_config.get("top_k", 20))
            dense_scan_chunk_size = int(lidar_config.get("chunk_size", 2000))

            # Env overrides (highest priority)
            dense_scan_enabled = os.getenv("LIDAR_DENSE_SCAN_ENABLED", str(dense_scan_enabled)).lower() == "true"
            dense_scan_points = int(os.getenv("LIDAR_DENSE_SCAN_POINTS", str(dense_scan_points)))
            dense_scan_radius_m = int(os.getenv("LIDAR_DENSE_SCAN_RADIUS_M", str(dense_scan_radius_m)))
            dense_scan_top_k = int(os.getenv("LIDAR_DENSE_SCAN_TOP_K", str(dense_scan_top_k)))
            dense_scan_chunk_size = int(os.getenv("LIDAR_DENSE_SCAN_CHUNK_SIZE", str(dense_scan_chunk_size)))
            ...
            if self.use_lidar_first and self.lidar_batch_processor and all_candidates:
                ...
                chunk_size = dense_scan_chunk_size if dense_scan_used else len(all_candidates)
                chunk_size = max(1, chunk_size)

                try:
                    for start_idx in range(0, len(all_candidates), chunk_size):
                        chunk = all_candidates[start_idx:start_idx + chunk_size]
                        batch_coords = [(c['lat'], c['lon']) for c in chunk]
                        batch_terrain = self.lidar_batch_processor.batch_extract(batch_coords, sample_radius_m=30)

                        total_hits += sum(1 for t in batch_terrain.values() if t and t.get('coverage'))
            ...
            if dense_scan_used:
                terrain_threshold = int(lidar_config.get("terrain_threshold", 0))
                terrain_threshold = int(os.getenv("LIDAR_DENSE_SCAN_TERRAIN_THRESHOLD", str(terrain_threshold)))
                max_gee_queries = max(3, dense_scan_top_k)
                promising_candidates = [
                    c for c in terrain_scored_candidates
                    if c['terrain_score'] >= terrain_threshold
                ][:max_gee_queries]
                decluster_m = int(lidar_config.get("min_separation_m", 75))
                decluster_m = int(os.getenv("LIDAR_DENSE_SCAN_MIN_SEPARATION_M", str(decluster_m)))
                if decluster_m > 0:
                    promising_candidates = self._decluster_candidates(
                        promising_candidates,
                        min_separation_m=decluster_m,
                        max_keep=max_gee_queries
                    )
            else:
            ...
            self.last_scan_trace = {
                "dense_scan_enabled": dense_scan_used,
                "candidates_generated": total_candidates,
                "lidar_coverage_hits": lidar_coverage_hits,
                "lidar_coverage_pct": round(coverage_pct, 2),
