...
class MatureBuckPointsGenerator:
    ...
    def generate_optimized_points(self, prediction_data: Dict[str, Any],
        ...
        logger.info(f" DEBUG score_maps keys: {list(score_maps.keys())}")
        ...
        logger.info(f" Using {len(bedding_features)} real bedding zones from enhanced predictor")
        ...
        try:
            ...
            logger.info(f"[OK] Generated {len(bedding_sites)} bedding, {len(stand_sites)} interception stands, "
    ...
    def _generate_bedding_sites(self, coordinates: np.ndarray, score_maps: Dict[str, np.ndarray],
        ...
        logger.debug(" Generating bedding sites from real zones")
        ...
        if bedding_features and len(bedding_features) > 0:
            ...
            logger.info(f"[OK] Using {len(bedding_features)} REAL bedding zones from LIDAR-enhanced predictor")
            ...
            logger.info(f"[OK] Generated {len(bedding_sites)} bedding sites from REAL zones")
        ...
        logger.warning(" No real bedding zones available, falling back to grid method")
    ...
    def _generate_interception_stands(self, coordinates: np.ndarray, score_maps: Dict[str, np.ndarray],
        ...
        logger.debug("Generating interception stands using full analysis")
    ...
    def _generate_single_best_feeding_site(self, coordinates: np.ndarray, score_maps: Dict[str, np.ndarray],
        ...
        logger.debug(" Generating best feeding site within 1500 feet")
