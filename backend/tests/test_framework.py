...
def test_config():
    ...
    config.api.openweathermap_api_key = "test_api_key"
    return config
...
class TestConfiguration:
    ...
    @pytest.mark.parametrize("api_key,should_warn", [
        ("", False),  # Empty key doesn't warn about length
        ("short", True),  # Short key should warn
        ("a" * 32, False),  # Correct length shouldn't warn
        ("a" * 40, True),  # Wrong length should warn
    ])
    def test_api_key_validation(self, api_key, should_warn):
